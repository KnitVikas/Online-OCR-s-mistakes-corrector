...
def get_probability_of_corret_character_replacement(operations_on_characters):      
    ...
    operations_sorted_index = sorted(operations_on_characters,key = lambda tuple_: tuple_[0])
    ...
    if len(list_character_to_check) == 1 and len(list_character_to_check[0])==1:
            ...
            if len(list_) == 1:
                ...
                for (index1, operation1, character1) , (index2, operation2, character2) in zip(list_, list_[1:]):
                    if operation1 == operation2:
                        ...
                        if (first_character) and (second_character) :
                            ...
                            print("either first_character or second_chacter are not satisfying the condition when operations are equal")
                    else:
                        if not (first_character):
                        ...
                        if first_character and not second_character:
                            ...
                            print("either first_character or second_chacter are not satisfying the condition when operations are  not equal")
                ...
                if first_character and second_character:
                    if is_in_same_list(first_character, second_character): 
                        ...
                        list_character_length_and_boolean_values.append((len(first_character), True))
                    else:
                        # print("replace character not matched in replace_char_list ")
                        list_character_length_and_boolean_values.append((len(first_character), False))
                else:
        ...
        if list_character_length_and_boolean_values:
            probability_of_correct_characters_updated = len([idx for idx in list_character_length_and_boolean_values  if idx[1]==True ])/len(list_character_length_and_boolean_values)     
...
def get_word_with_probability_and_edit_distance(correct_word_list):
    ...
    sorted_word_probability = sorted(word_probability,key = lambda element:(element[0],element[1]))    
    sorted_best_matches_list = [tuple_ for tuple_ in sorted_word_probability if tuple_[0]==sorted_word_probability[0][0]]
...
if __name__== "__main__":
    ...
    white_list_words = ['place', 'mark', 'part', 'tariff', 'quantity', 'packages', 'description','information', 'gross', 'class', 'hazmat', 'commodity', 'package', 'pallet', 'value', 'marks', 'pieces', 'type', 'parties', 'order', 'volume', 'weight', 'numeric', 'division', 'item', 'shipping', 'product', 'slip', 'batch', 'partial', 'expiration', 'unit', 'details', 'measurement', 'count', 'nature', 'container', 'price', 'rate', 'charge', 'packaging', 'group', 'ordered', 'packs', 'goods', 'amount', 'hash', 'chargeable', 'tons', 'total', 'serial', 'descending']
