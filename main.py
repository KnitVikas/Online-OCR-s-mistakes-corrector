...
def is_in_same_list(char1, char2):

    for list_ in char_replace_list: 
        if char1 in list_ and char2  in list_:
            return True
    return False

def get_probability_of_corret_character_replacement(operations):
      
    # sort the operation on the basis of index of character
    operations_sorted_index=sorted(operations,key = lambda tup: tup[0])
    ...
    list_char_to_check = list()
    last_idx = 0
    for index in split_at_index:
        new_list = [ idx for idx in operations_sorted_index[last_idx:] if idx[0] < index]
        list_char_to_check.append(new_list)
        last_idx += len(new_list)
    list_char_to_check.append([ x for x in operations_sorted_index[last_idx:]])
    print(list_char_to_check)
    ...
    if len(list_char_to_check)==1 and len(list_char_to_check[0])==1:
        return 0.0
    else:
        list_tup_length_bool_value=[]
        for list_ in list_char_to_check:
            if len(list_)==1:
                list_tup_length_bool_value.append((1,False))
                ...
                if first_character and second_character:
                    if is_in_same_list(first_character,second_character) : 
                        ...
                        list_tup_length_bool_value.append((len(first_character), True))
                    else:
                        print("replace character not matched in replace_char_list ")
                        list_tup_length_bool_value.append((len(first_character),False))
                else:
                    print("any of first and second character is empty ")
                        # ''' work on list_tup_length_bool_value '''
            
        print("list_tup_length_bool_value",list_tup_length_bool_value)
        if list_tup_length_bool_value:
            probability = len([idx for idx in list_tup_length_bool_value  if idx[1]==True ])/len(list_tup_length_bool_value)     
            return probability
        else :
            return 0.0
...
def get_word_with_probability_and_edit_distance(correct_word_list):
    ...
    list_correct_small_words=[word for word in correct_word_list if 3<len(word)<7 ]
    ...
    if  list_correct_small_words :
        ...
        for word in best_match_small_word_edited_distance:
            if word[0]<=2:
                ...
                probability=get_probability_of_corret_character_replacement(operations_on_characters)
                print("probability",probability)
                word_probability.append((word[0],probability,word[1]))
            else:
                print("edit distance is much bigger")
    else:
    ...
    if list_correct_long_words:        
        ...
        for word in best_match_long_word_edited_distance:
            if word[0]<=4:
                operations_on_characters=get_operations_on_characters([(incorrect_word,word[1])])
                probability = get_probability_of_corret_character_replacement(operations_on_characters)
                print("probability long",probability)
                word_probability.append((word[0],probability,word[1]))
            else:
                print("edit distance is much bigger")
    else:
    ...
    #print("No suggested words")
    sorted_best_matches_list=sorted(word_probability,key = lambda element:(element[0],element[1]))[0]
    print("sorted_best_matches_list",word_probability)
...
def get_final_similar_word(white_list_words,incorrect_word,incorrect_word_embedding,white_list_word_embeddings):
        ...
        # if common_words exist
            if common_words:
                ...
                best_matched_words_syms = get_word_with_probability_and_edit_distance(matched_words_syms)
                best_matched_words_char2vec = get_word_with_probability_and_edit_distance(matched_words_char2vec)
